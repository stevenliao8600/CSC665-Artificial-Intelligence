...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        if len(nextLegalActions) == 0:
          ...
          nextQVal = max([self.getQValue(nextState, nextAction) for nextAction in nextLegalActions])
        self.qValues[state, action] = (1 - self.alpha) * self.getQValue(state, action) + self.alpha * (reward + self.discount * nextQVal)
